/**
 * Pure core of the api-routes generator: routes.yaml text in, the full
 * generated api-routes.ts file content out. No filesystem access — the CLI
 * runner (generate-api-routes.ts) does the reading/writing, and the drift
 * guard (src/cli/commands/api-routes.drift.spec.ts) calls this directly to
 * prove the checked-in file matches its source of truth.
 */
...
export function generateFile(routes: ParsedRoute[]): string {
...
/** routes.yaml text in → complete api-routes.ts file content out. */
export function buildApiRoutesFile(yamlRaw: string): string {
