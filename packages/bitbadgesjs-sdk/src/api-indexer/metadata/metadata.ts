 ...
 * @category Interfaces
 */
export type iMetadataWithoutInternals = Omit<iMetadata, '_isUpdating' | 'fetchedAt' | 'fetchedAtBlock'>;

/**
 * @inheritDoc iMetadata
 * @category Collections
 */
export class Metadata extends BaseNumberTypeClass<Metadata> implements iMetadata {
  fetchedAt?: UNIXMilliTimestamp;
  fetchedAtBlock?: string | number;
  _isUpdating?: boolean;
  name: string;
  ...
  convert(convertFunction: (item: string | number) => U, options?: ConvertOptions): Metadata {
    return convertClassPropertiesAndMaintainNumberTypes(this, convertFunction, options) as Metadata;
  }
