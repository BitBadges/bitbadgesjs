 ...
 * @category Interfaces
 */
export type NotificationType = 'transfer' | 'bank_send' | 'claim' | 'list' | 'system' | 'intent_satisfied';

/**
 * A coin amount in base units, encoded as a decimal integer string.
 *
 * @category Interfaces
 */
...
export interface iNotificationPayload<T extends NumberType> {
  /** `transfer`: public source activity for rich rendering. Omitted for private transfers. */
  ...
  intentSatisfied?: {
    ...
    txHash?: string;
    /** Coins that moved as part of the fill (payment/payout), in base units. */
  ...
  /** Misc display-ready scalars for any type (tokenIds, claimId, …). */
