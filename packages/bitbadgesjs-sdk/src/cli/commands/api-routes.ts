// AUTO-GENERATED by scripts/generate-api-routes.ts — regenerate with: bun run generate-api-routes
// Do not edit this file manually. Changes will be overwritten.

// ---------------------------------------------------------------------------
// Types
// ---------------------------------------------------------------------------

export interface SdkLinks {
  /** SDK request/payload type name */
  request?: string;
  /** SDK response type name */
  response?: string;
  /** SDK API function name */
  function?: string;
}

export interface ParamInfo {
  name: string;
  description: string;
  required: boolean;
}

export interface FieldInfo {
  name: string;
  type: string;
  description: string;
  required: boolean;
}

export interface ApiRoute {
  /** CLI command name (kebab-case) */
  name: string;
  /** HTTP method */
  method: 'GET' | 'POST' | 'PUT' | 'DELETE';
  /** API path template with {param} placeholders */
  path: string;
  /** Short description shown in --help */
  description: string;
  /** Ordered list of path parameters (maps to positional CLI args) */
  pathParams: string[];
  /** Whether the route accepts a JSON request body */
  hasBody: boolean;
  /** Tag for grouping in CLI help */
  tag: string;
  /** Query parameters (for GET routes with exploded payload) */
  queryParams?: ParamInfo[];
  /** Key body fields (for POST/PUT/DELETE routes) */
  bodyFields?: FieldInfo[];
  /** SDK usage example */
  example?: string;
  /** Links to SDK type docs */
  sdkLinks?: SdkLinks;
  /** Compact TypeScript-like schema for the request type */
  requestSchema?: string;
  /** Compact TypeScript-like schema for the response type */
  responseSchema?: string;
}

// ---------------------------------------------------------------------------
// Tag descriptions
// ---------------------------------------------------------------------------

export const TAG_DESCRIPTIONS: Record<string, string> = {
  'accounts': 'Account and user routes',
  'tokens': 'Collection and token routes',
  'claims': 'Claim management routes',
  'auth': 'Sign In with BitBadges / OAuth routes',
  'tx': 'Transaction broadcast and simulation',
  'apps': 'Developer apps and applications',
  'plugins': 'Plugin management routes',
  'stores': 'Dynamic data store routes',
  'onchain-stores': 'On-chain dynamic store routes',
  'pages': 'Utility page routes',
  'maps': 'On-chain map and protocol routes',
  'assets': 'DEX, pools, and asset pair routes',
  'misc': 'Miscellaneous routes',
};

// ---------------------------------------------------------------------------
// Routes (140 total)
// ---------------------------------------------------------------------------

export const ROUTES: ApiRoute[] = [
  // =========================================================================
  // Account and user routes
  // =========================================================================
  {
    name: 'get-account',
    tag: 'accounts',
    method: 'GET',
    path: '/user',
    description: 'Get Account',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iGetAccountPayload',
      response: 'iGetAccountSuccessResponse',
      function: 'BitBadgesAPI.getAccount',
    },
  },
  {
    name: 'get-accounts',
    tag: 'accounts',
    method: 'POST',
    path: '/users',
    description: 'Get Accounts - Batch',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetAccountsPayload',
      response: 'iGetAccountsSuccessResponse',
      function: 'BitBadgesAPI.getAccounts',
    },
  },
  {
    name: 'get-siwbb-requests-for-user',
    tag: 'accounts',
    method: 'GET',
    path: '/account/{address}/requests/siwbb',
    description: 'Get SIWBB Requests For User',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSiwbbRequestsForUserPayload',
      response: 'iGetSiwbbRequestsForUserSuccessResponse',
      function: 'BitBadgesAPI.getSiwbbRequestsForUser',
    },
  },
  {
    name: 'get-transfer-activity-for-user',
    tag: 'accounts',
    method: 'GET',
    path: '/account/{address}/activity/tokens',
    description: 'Get Transfer Activity For User',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetTransferActivityForUserPayload',
      response: 'iGetTransferActivityForUserSuccessResponse',
      function: 'BitBadgesAPI.getTransferActivityForUser',
    },
  },
  {
    name: 'get-tokens-view-for-user',
    tag: 'accounts',
    method: 'GET',
    path: '/account/{address}/tokens',
    description: 'Get Tokens For User',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetTokensViewForUserPayload',
      response: 'iGetTokensViewForUserSuccessResponse',
      function: 'BitBadgesAPI.GetTokensViewForUser',
    },
  },
  {
    name: 'get-user-balances',
    tag: 'accounts',
    method: 'GET',
    path: '/account/{address}/balances',
    description: 'Get User Balances',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetUserBalancesPayload',
      response: 'iGetUserBalancesSuccessResponse',
      function: 'BitBadgesAPI.getUserBalances',
    },
  },
  {
    name: 'get-claim-activity-for-user',
    tag: 'accounts',
    method: 'GET',
    path: '/account/{address}/activity/claims',
    description: 'Get Claim Activity For User',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetClaimActivityForUserPayload',
      response: 'iGetClaimActivityForUserSuccessResponse',
      function: 'BitBadgesAPI.getClaimActivityForUser',
    },
  },
  {
    name: 'get-points-activity-for-user',
    tag: 'accounts',
    method: 'GET',
    path: '/account/{address}/activity/points',
    description: 'Get Points Activity For User',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetPointsActivityForUserPayload',
      response: 'iGetPointsActivityForUserSuccessResponse',
      function: 'BitBadgesAPI.getPointsActivityForUser',
    },
  },
  {
    name: 'get-votes-by-voter',
    tag: 'accounts',
    method: 'GET',
    path: '/voter/{voter}/votes',
    description: 'Get Votes By Voter',
    pathParams: ['voter'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetVotesByVoterPayload',
      response: 'iGetVotesByVoterSuccessResponse',
      function: 'BitBadgesAPI.getVotesByVoter',
    },
  },
  // =========================================================================
  // Collection and token routes
  // =========================================================================
  {
    name: 'get-collection',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}',
    description: 'Get Collection',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetCollectionSuccessResponse',
      function: 'BitBadgesAPI.getCollection',
    },
  },
  {
    name: 'get-token-metadata',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/{tokenId}/metadata',
    description: 'Get Token Metadata',
    pathParams: ['collectionId', 'tokenId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetTokenMetadataSuccessResponse',
      function: 'BitBadgesAPI.getTokenMetadata',
    },
  },
  {
    name: 'get-collections-batch',
    tag: 'tokens',
    method: 'POST',
    path: '/collections',
    description: 'Get Collections - Batch',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetCollectionsPayload',
      response: 'iGetCollectionsSuccessResponse',
      function: 'BitBadgesAPI.getCollectionsBatch',
    },
  },
  {
    name: 'get-balance-by-address-specific-token',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/{tokenId}/balance/{address}',
    description: 'Get Balance By Address - Specific Token',
    pathParams: ['collectionId', 'address', 'tokenId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetBalanceByAddressSpecificTokenSuccessResponse',
      function: 'BitBadgesAPI.getBalanceByAddressSpecificToken',
    },
  },
  {
    name: 'get-balance-by-address',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/balance/{address}',
    description: 'Get Balances By Address',
    pathParams: ['collectionId', 'address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetBalanceByAddressPayload',
      response: 'iGetBalanceByAddressSuccessResponse',
      function: 'BitBadgesAPI.getBalanceByAddress',
    },
  },
  {
    name: 'get-owners',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/{tokenId}/owners',
    description: 'Get Token Owners',
    pathParams: ['collectionId', 'tokenId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetOwnersPayload',
      response: 'iGetOwnersSuccessResponse',
      function: 'BitBadgesAPI.getOwners',
    },
  },
  {
    name: 'get-token-activity',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/{tokenId}/activity',
    description: 'Get Token Activity',
    pathParams: ['collectionId', 'tokenId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetTokenActivityPayload',
      response: 'iGetTokenActivitySuccessResponse',
      function: 'BitBadgesAPI.getTokenActivity',
    },
  },
  {
    name: 'refresh-metadata',
    tag: 'tokens',
    method: 'POST',
    path: '/collection/{collectionId}/refresh',
    description: 'Refresh Metadata',
    pathParams: ['collectionId'],
    hasBody: true,
    sdkLinks: {
      request: 'iRefreshMetadataPayload',
      response: 'iRefreshMetadataSuccessResponse',
      function: 'BitBadgesAPI.refreshMetadata',
    },
  },
  {
    name: 'get-refresh-status',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/refreshStatus',
    description: 'Get Refresh Status',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      response: 'iRefreshStatusSuccessResponse',
      function: 'BitBadgesAPI.getRefreshStatus',
    },
  },
  {
    name: 'get-collection-owners',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/owners',
    description: 'Get Collection Owners',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetCollectionOwnersPayload',
      response: 'iGetCollectionOwnersSuccessResponse',
      function: 'BitBadgesAPI.getCollectionOwners',
    },
  },
  {
    name: 'get-collection-transfer-activity',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/activity',
    description: 'Get Collection Transfer Activity',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetCollectionTransferActivityPayload',
      response: 'iGetCollectionTransferActivitySuccessResponse',
      function: 'BitBadgesAPI.getCollectionTransferActivity',
    },
  },
  {
    name: 'get-collection-challenge-trackers',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/challengeTrackers',
    description: 'Get Collection Challenge Trackers',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetCollectionChallengeTrackersPayload',
      response: 'iGetCollectionChallengeTrackersSuccessResponse',
      function: 'BitBadgesAPI.getCollectionChallengeTrackers',
    },
  },
  {
    name: 'get-collection-amount-trackers',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/amountTrackers',
    description: 'Get Collection Amount Trackers',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetCollectionAmountTrackersPayload',
      response: 'iGetCollectionAmountTrackersSuccessResponse',
      function: 'BitBadgesAPI.getCollectionAmountTrackers',
    },
  },
  {
    name: 'get-collection-amount-tracker-by-id',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/amountTracker',
    description: 'Get Collection Amount Tracker By ID',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iAmountTrackerIdDetails',
      response: 'iGetCollectionAmountTrackerByIdSuccessResponse',
      function: 'BitBadgesAPI.getCollectionAmountTrackerById',
    },
  },
  {
    name: 'get-collection-challenge-tracker-by-id',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/challengeTracker',
    description: 'Get Collection Challenge Tracker By ID',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iChallengeTrackerIdDetails',
      response: 'iGetCollectionChallengeTrackerByIdSuccessResponse',
      function: 'BitBadgesAPI.getCollectionChallengeTrackerById',
    },
  },
  {
    name: 'get-collection-listings',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/listings',
    description: 'Get Collection Listings',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetCollectionListingsPayload',
      response: 'iGetCollectionListingsSuccessResponse',
      function: 'BitBadgesAPI.getCollectionListings',
    },
  },
  {
    name: 'get-collection-claims',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/claims',
    description: 'Get Collection Claims',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetCollectionClaimsSuccessResponse',
      function: 'BitBadgesAPI.getCollectionClaims',
    },
  },
  {
    name: 'filter-tokens',
    tag: 'tokens',
    method: 'POST',
    path: '/collection/{collectionId}/filter',
    description: 'Filter Tokens in Collection',
    pathParams: ['collectionId'],
    hasBody: true,
    sdkLinks: {
      request: 'iFilterTokensInCollectionPayload',
      response: 'iFilterTokensInCollectionSuccessResponse',
      function: 'BitBadgesAPI.filterTokens',
    },
  },
  {
    name: 'get-filter-suggestions',
    tag: 'tokens',
    method: 'POST',
    path: '/collection/{collectionId}/filterSuggestions',
    description: 'Get Filter Suggestions',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      response: 'iFilterSuggestionsSuccessResponse',
      function: 'BitBadgesAPI.getFilterSuggestions',
    },
  },
  {
    name: 'filter-collection-approvals',
    tag: 'tokens',
    method: 'POST',
    path: '/collection/{collectionId}/filterApprovals',
    description: 'Filter Collection Approvals',
    pathParams: ['collectionId'],
    hasBody: true,
    sdkLinks: {
      request: 'iFilterCollectionApprovalsPayload',
      response: 'iFilterCollectionApprovalsSuccessResponse',
      function: 'BitBadgesAPI.filterCollectionApprovals',
    },
  },
  {
    name: 'get-all-listings',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/allListings',
    description: 'Get All Listings for Collection',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetAllListingsPayload',
      response: 'iGetAllListingsSuccessResponse',
      function: 'BitBadgesAPI.getAllListings',
    },
  },
  {
    name: 'get-collection-offers',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/collectionOffers',
    description: 'Get Collection-Wide Offers',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetCollectionOffersPayload',
      response: 'iGetCollectionOffersSuccessResponse',
      function: 'BitBadgesAPI.getCollectionOffers',
    },
  },
  {
    name: 'get-listings-for-token-id',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/listings/{tokenId}',
    description: 'Get Listings For Token',
    pathParams: ['collectionId', 'tokenId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetListingsForTokenIdPayload',
      response: 'iGetListingsForTokenIdSuccessResponse',
      function: 'BitBadgesAPI.getListingsForTokenId',
    },
  },
  {
    name: 'get-offers-for-token-id',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/offers/{tokenId}',
    description: 'Get Offers For Token',
    pathParams: ['collectionId', 'tokenId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetOffersForTokenIdPayload',
      response: 'iGetOffersForTokenIdSuccessResponse',
      function: 'BitBadgesAPI.getOffersForTokenId',
    },
  },
  {
    name: 'get-orderbook-depth',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/orderbook/{tokenId}',
    description: 'Get Orderbook Depth',
    pathParams: ['collectionId', 'tokenId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetOrderbookDepthPayload',
      response: 'iGetOrderbookDepthSuccessResponse',
      function: 'BitBadgesAPI.getOrderbookDepth',
    },
  },
  {
    name: 'get-candlestick-data',
    tag: 'tokens',
    method: 'POST',
    path: '/collection/{collectionId}/candlestick/{tokenId}',
    description: 'Get Candlestick Data',
    pathParams: ['collectionId', 'tokenId'],
    hasBody: true,
    sdkLinks: {
      request: 'iGetCandlestickDataPayload',
      response: 'iGetCandlestickDataSuccessResponse',
      function: 'BitBadgesAPI.getCandlestickData',
    },
  },
  {
    name: 'get-votes-by-collection',
    tag: 'tokens',
    method: 'GET',
    path: '/collection/{collectionId}/votes',
    description: 'Get Votes By Collection',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetVotesByCollectionPayload',
      response: 'iGetVotesByCollectionSuccessResponse',
      function: 'BitBadgesAPI.getVotesByCollection',
    },
  },
  {
    name: 'get-vote-by-proposal-id',
    tag: 'tokens',
    method: 'GET',
    path: '/vote/{proposalId}',
    description: 'Get Vote By Proposal ID',
    pathParams: ['proposalId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetVoteByProposalIdSuccessResponse',
      function: 'BitBadgesAPI.getVoteByProposalId',
    },
  },
  {
    name: 'get-predictions',
    tag: 'tokens',
    method: 'GET',
    path: '/predictions',
    description: 'Browse Prediction Markets',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      response: 'iGetPredictionsSuccessResponse',
      function: 'BitBadgesAPI.getPredictions',
    },
  },
  {
    name: 'get-prediction-detail',
    tag: 'tokens',
    method: 'GET',
    path: '/predictions/{collectionId}',
    description: 'Get Prediction Market Detail',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetPredictionDetailSuccessResponse',
      function: 'BitBadgesAPI.getPredictionDetail',
    },
  },
  {
    name: 'get-prediction-prices',
    tag: 'tokens',
    method: 'GET',
    path: '/predictions/{collectionId}/prices',
    description: 'Get Prediction Market Price History',
    pathParams: ['collectionId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetPredictionPricesPayload',
      response: 'iGetPredictionPricesSuccessResponse',
      function: 'BitBadgesAPI.getPredictionPrices',
    },
  },
  // =========================================================================
  // Claim management routes
  // =========================================================================
  {
    name: 'get-claim',
    tag: 'claims',
    method: 'GET',
    path: '/claim/{claimId}',
    description: 'Get Claim',
    pathParams: ['claimId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetClaimPayload',
      response: 'iGetClaimSuccessResponse',
      function: 'BitBadgesAPI.getClaim',
    },
  },
  {
    name: 'complete-claim',
    tag: 'claims',
    method: 'POST',
    path: '/claims/complete/{claimId}/{address}',
    description: 'Complete Claim',
    pathParams: ['claimId', 'address'],
    hasBody: true,
    sdkLinks: {
      request: 'iCompleteClaimPayload',
      response: 'iCompleteClaimSuccessResponse',
      function: 'BitBadgesAPI.completeClaim',
    },
  },
  {
    name: 'simulate-claim',
    tag: 'claims',
    method: 'POST',
    path: '/claims/simulate/{claimId}/{address}',
    description: 'Simulate Claim',
    pathParams: ['claimId', 'address'],
    hasBody: true,
    sdkLinks: {
      request: 'iSimulateClaimPayload',
      response: 'iSimulateClaimSuccessResponse',
      function: 'BitBadgesAPI.simulateClaim',
    },
  },
  {
    name: 'get-reserved-codes',
    tag: 'claims',
    method: 'POST',
    path: '/claims/reserved/{claimId}/{address}',
    description: 'Get Reserved Claim Codes',
    pathParams: ['claimId', 'address'],
    hasBody: true,
    sdkLinks: {
      request: 'iGetReservedClaimCodesPayload',
      response: 'iGetReservedClaimCodesSuccessResponse',
      function: 'BitBadgesAPI.getReservedCodes',
    },
  },
  {
    name: 'get-claim-attempt-status',
    tag: 'claims',
    method: 'GET',
    path: '/claims/status/{claimAttemptId}',
    description: 'Get Claim Attempt Status',
    pathParams: ['claimAttemptId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetClaimAttemptStatusPayload',
      response: 'iGetClaimAttemptStatusSuccessResponse',
      function: 'BitBadgesAPI.getClaimAttemptStatus',
    },
  },
  {
    name: 'search-claims',
    tag: 'claims',
    method: 'GET',
    path: '/claims/search',
    description: 'Search Claims',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iSearchClaimsPayload',
      response: 'iSearchClaimsSuccessResponse',
      function: 'BitBadgesAPI.searchClaims',
    },
  },
  {
    name: 'get-claims',
    tag: 'claims',
    method: 'POST',
    path: '/claims/fetch',
    description: 'Get Claims - Batch',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetClaimsPayloadV1',
      response: 'iGetClaimsSuccessResponse',
      function: 'BitBadgesAPI.getClaims',
    },
  },
  {
    name: 'create-claim',
    tag: 'claims',
    method: 'POST',
    path: '/claims',
    description: 'Create Claim',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCreateClaimPayload',
      response: 'iCreateClaimSuccessResponse',
      function: 'BitBadgesAPI.createClaim',
    },
  },
  {
    name: 'update-claim',
    tag: 'claims',
    method: 'PUT',
    path: '/claims',
    description: 'Update Claim',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iUpdateClaimPayload',
      response: 'iUpdateClaimSuccessResponse',
      function: 'BitBadgesAPI.updateClaim',
    },
  },
  {
    name: 'delete-claim',
    tag: 'claims',
    method: 'DELETE',
    path: '/claims',
    description: 'Delete Claim',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iDeleteClaimPayload',
      response: 'iDeleteClaimSuccessResponse',
      function: 'BitBadgesAPI.deleteClaim',
    },
  },
  {
    name: 'generate-code',
    tag: 'claims',
    method: 'GET',
    path: '/codes',
    description: 'Get Code (Codes Plugin)',
    pathParams: [],
    hasBody: false,
    queryParams: [
      { name: 'seedCode', description: 'The seed used to generate the code', required: true },
      { name: 'idx', description: 'The index of the code to generate', required: true },
    ],
    sdkLinks: {
      function: 'BitBadgesAPI.generateCode',
    },
  },
  {
    name: 'get-claim-attempts',
    tag: 'claims',
    method: 'GET',
    path: '/claims/{claimId}/attempts',
    description: 'Get Claim Attempts',
    pathParams: ['claimId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetClaimAttemptsPayload',
      response: 'iGetClaimAttemptsSuccessResponse',
      function: 'BitBadgesAPI.getClaimAttempts',
    },
  },
  {
    name: 'get-gated-content-for-claim',
    tag: 'claims',
    method: 'GET',
    path: '/claims/gatedContent/{claimId}',
    description: 'Get Gated Content for Claim',
    pathParams: ['claimId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetGatedContentForClaimPayload',
      response: 'iGetGatedContentForClaimSuccessResponse',
      function: 'BitBadgesAPI.getGatedContentForClaim',
    },
  },
  // =========================================================================
  // Sign In with BitBadges / OAuth routes
  // =========================================================================
  {
    name: 'get-developer-app',
    tag: 'auth',
    method: 'GET',
    path: '/developerApp/{clientId}',
    description: 'Get OAuth App',
    pathParams: ['clientId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetDeveloperAppPayload',
      response: 'iGetDeveloperAppSuccessResponse',
      function: 'BitBadgesAPI.getDeveloperApp',
    },
  },
  {
    name: 'create-developer-app',
    tag: 'auth',
    method: 'POST',
    path: '/developerApps',
    description: 'Create OAuth App',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      response: 'iCreateDeveloperAppSuccessResponse',
      function: 'BitBadgesAPI.createDeveloperApp',
    },
  },
  {
    name: 'update-developer-app',
    tag: 'auth',
    method: 'PUT',
    path: '/developerApps',
    description: 'Update OAuth App',
    pathParams: ['clientId'],
    hasBody: false,
    sdkLinks: {
      response: 'iUpdateDeveloperAppSuccessResponse',
      function: 'BitBadgesAPI.updateDeveloperApp',
    },
  },
  {
    name: 'delete-developer-app',
    tag: 'auth',
    method: 'DELETE',
    path: '/developerApps',
    description: 'Delete OAuth App',
    pathParams: ['clientId'],
    hasBody: false,
    sdkLinks: {
      response: 'iDeleteDeveloperAppSuccessResponse',
      function: 'BitBadgesAPI.deleteDeveloperApp',
    },
  },
  {
    name: 'exchange-siwbb-authorization-code',
    tag: 'auth',
    method: 'POST',
    path: '/siwbb/token',
    description: 'Exchange SIWBB Code',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iExchangeSIWBBAuthorizationCodePayload',
      response: 'iExchangeSIWBBAuthorizationCodeSuccessResponse',
      function: 'BitBadgesAPI.exchangeSIWBBAuthorizationCode',
    },
  },
  {
    name: 'revoke-oauth-authorization',
    tag: 'auth',
    method: 'POST',
    path: '/siwbb/token/revoke',
    description: 'Revoke Authorization',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iOauthRevokePayload',
      response: 'iOauthRevokeSuccessResponse',
      function: 'BitBadgesAPI.revokeOauthAuthorization',
    },
  },
  {
    name: 'rotate-siwbb-request',
    tag: 'auth',
    method: 'POST',
    path: '/siwbbRequest/rotate',
    description: 'Rotate SIWBB Request',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iRotateSIWBBRequestPayload',
      response: 'iRotateSIWBBRequestSuccessResponse',
      function: 'BitBadgesAPI.rotateSIWBBRequest',
    },
  },
  {
    name: 'delete-siwbb-request',
    tag: 'auth',
    method: 'DELETE',
    path: '/siwbbRequest',
    description: 'Delete SIWBB Request',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iDeleteSIWBBRequestPayload',
      response: 'iDeleteSIWBBRequestSuccessResponse',
      function: 'BitBadgesAPI.deleteSIWBBRequest',
    },
  },
  {
    name: 'create-siwbb-request',
    tag: 'auth',
    method: 'POST',
    path: '/siwbbRequest',
    description: 'Create SIWBB Request',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCreateSIWBBRequestPayload',
      response: 'iCreateSIWBBRequestSuccessResponse',
      function: 'BitBadgesAPI.createSIWBBRequest',
    },
  },
  {
    name: 'get-siwbb-requests-for-developer-app',
    tag: 'auth',
    method: 'GET',
    path: '/developerApps/siwbbRequests',
    description: 'Get SIWBB Requests For Developer App',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSIWBBRequestsForDeveloperAppPayload',
      response: 'iGetSIWBBRequestsForDeveloperAppSuccessResponse',
      function: 'BitBadgesAPI.getSIWBBRequestsForDeveloperApp',
    },
  },
  {
    name: 'check-sign-in-status',
    tag: 'auth',
    method: 'POST',
    path: '/auth/status',
    description: 'Check Sign In Status',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCheckSignInStatusPayload',
      response: 'iCheckSignInStatusSuccessResponse',
      function: 'BitBadgesAPI.checkSignInStatus',
    },
  },
  {
    name: 'search-developer-apps',
    tag: 'auth',
    method: 'GET',
    path: '/developerApps/search',
    description: 'Search Developer Apps',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iSearchDeveloperAppsPayload',
      response: 'iSearchDeveloperAppsSuccessResponse',
      function: 'BitBadgesAPI.searchDeveloperApps',
    },
  },
  {
    name: 'fetch-developer-apps',
    tag: 'auth',
    method: 'POST',
    path: '/developerApps/fetch',
    description: 'Fetch Developer Apps (Batch)',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetDeveloperAppsPayload',
      response: 'iGetDeveloperAppsSuccessResponse',
      function: 'BitBadgesAPI.fetchDeveloperApps',
    },
  },
  // =========================================================================
  // Transaction broadcast and simulation
  // =========================================================================
  {
    name: 'broadcast-tx',
    tag: 'tx',
    method: 'POST',
    path: '/broadcast',
    description: 'Broadcast a transaction via the indexer API. NOTE: If using the chain CLI, prefer \`bitbadgeschaind tx\` for direct node broadcast.',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      response: 'iBroadcastTxSuccessResponse',
      function: 'BitBadgesAPI.broadcastTx',
    },
  },
  {
    name: 'simulate-tx',
    tag: 'tx',
    method: 'POST',
    path: '/simulate',
    description: 'Simulate a transaction via the indexer API. NOTE: If using the chain CLI, prefer \`bitbadgeschaind tx ... --dry-run\` for direct node simulation.',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      response: 'iSimulateTxSuccessResponse',
      function: 'BitBadgesAPI.simulateTx',
    },
  },
  {
    name: 'broadcast-tx-evm',
    tag: 'tx',
    method: 'POST',
    path: '/broadcast-evm',
    description: 'Broadcast EVM Transaction',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iBroadcastTxEvmPayload',
      response: 'iBroadcastTxEvmSuccessResponse',
      function: 'BitBadgesAPI.broadcastTxEvm',
    },
  },
  {
    name: 'simulate-tx-evm',
    tag: 'tx',
    method: 'POST',
    path: '/simulate-evm',
    description: 'Simulate EVM Transaction',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iSimulateTxEvmPayload',
      response: 'iSimulateTxEvmSuccessResponse',
      function: 'BitBadgesAPI.simulateTxEvm',
    },
  },
  // =========================================================================
  // Plugin management routes
  // =========================================================================
  {
    name: 'get-plugin',
    tag: 'plugins',
    method: 'GET',
    path: '/plugin/{pluginId}',
    description: 'Get Plugin',
    pathParams: ['pluginId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetPluginPayload',
      response: 'iGetPluginSuccessResponse',
      function: 'BitBadgesAPI.getPlugin',
    },
  },
  {
    name: 'get-plugins',
    tag: 'plugins',
    method: 'POST',
    path: '/plugins/fetch',
    description: 'Get Plugins - Batch',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetPluginsPayload',
      response: 'iGetPluginSuccessResponse',
      function: 'BitBadgesAPI.getPlugins',
    },
  },
  {
    name: 'search-plugins',
    tag: 'plugins',
    method: 'GET',
    path: '/plugins/search',
    description: 'Search Plugins',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iSearchPluginsPayload',
      response: 'iSearchPluginsSuccessResponse',
      function: 'BitBadgesAPI.searchPlugins',
    },
  },
  {
    name: 'get-creator-plugins',
    tag: 'plugins',
    method: 'GET',
    path: '/plugins/creator',
    description: 'Get Creator Plugins',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iGetCreatorPluginsPayload',
      response: 'iGetPluginSuccessResponse',
      function: 'BitBadgesAPI.getCreatorPlugins',
    },
  },
  {
    name: 'create-plugin',
    tag: 'plugins',
    method: 'POST',
    path: '/plugins',
    description: 'Create Plugin',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCreatePluginPayload',
      response: 'iCreatePluginSuccessResponse',
      function: 'BitBadgesAPI.createPlugin',
    },
  },
  {
    name: 'update-plugin',
    tag: 'plugins',
    method: 'PUT',
    path: '/plugins',
    description: 'Update Plugin',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iUpdatePluginPayload',
      response: 'iUpdatePluginSuccessResponse',
      function: 'BitBadgesAPI.updatePlugin',
    },
  },
  {
    name: 'delete-plugin',
    tag: 'plugins',
    method: 'DELETE',
    path: '/plugins',
    description: 'Delete Plugin',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iDeletePluginPayload',
      response: 'iDeletePluginSuccessResponse',
      function: 'BitBadgesAPI.deletePlugin',
    },
  },
  // =========================================================================
  // Dynamic data store routes
  // =========================================================================
  {
    name: 'get-dynamic-data-store',
    tag: 'stores',
    method: 'GET',
    path: '/dynamicStore/{dynamicStoreId}',
    description: 'Get Dynamic Data Store',
    pathParams: ['dynamicStoreId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetDynamicDataStorePayload',
      response: 'iGetDynamicDataStoreSuccessResponse',
      function: 'BitBadgesAPI.getDynamicDataStore',
    },
  },
  {
    name: 'get-dynamic-data-store-value',
    tag: 'stores',
    method: 'GET',
    path: '/dynamicStore/{dynamicStoreId}/value',
    description: 'Get Dynamic Data Store Value',
    pathParams: ['dynamicStoreId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetDynamicDataStoreValuePayload',
      response: 'iGetDynamicDataStoreValueSuccessResponse',
      function: 'BitBadgesAPI.getDynamicDataStoreValue',
    },
  },
  {
    name: 'get-dynamic-data-store-values-paginated',
    tag: 'stores',
    method: 'GET',
    path: '/dynamicStore/{dynamicStoreId}/values',
    description: 'Get Dynamic Data Store Values Paginated',
    pathParams: ['dynamicStoreId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetDynamicDataStoreValuesPaginatedPayload',
      response: 'iGetDynamicDataStoreValuesPaginatedSuccessResponse',
      function: 'BitBadgesAPI.getDynamicDataStoreValuesPaginated',
    },
  },
  {
    name: 'create-dynamic-data-store',
    tag: 'stores',
    method: 'POST',
    path: '/dynamicStores',
    description: 'Create Dynamic Data Store',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCreateDynamicDataStorePayload',
      response: 'iCreateDynamicDataStoreSuccessResponse',
      function: 'BitBadgesAPI.createDynamicDataStore',
    },
  },
  {
    name: 'update-dynamic-data-store',
    tag: 'stores',
    method: 'PUT',
    path: '/dynamicStores',
    description: 'Update Dynamic Data Store',
    pathParams: ['dynamicStoreId'],
    hasBody: true,
    sdkLinks: {
      request: 'iUpdateDynamicDataStorePayload',
      response: 'iUpdateDynamicDataStoreSuccessResponse',
      function: 'BitBadgesAPI.updateDynamicDataStore',
    },
  },
  {
    name: 'delete-dynamic-data-store',
    tag: 'stores',
    method: 'DELETE',
    path: '/dynamicStores',
    description: 'Delete Dynamic Data Store',
    pathParams: ['dynamicStoreId'],
    hasBody: true,
    sdkLinks: {
      request: 'iDeleteDynamicDataStorePayload',
      response: 'iDeleteDynamicDataStoreSuccessResponse',
      function: 'BitBadgesAPI.deleteDynamicDataStore',
    },
  },
  {
    name: 'perform-store-action-single-with-body-auth',
    tag: 'stores',
    method: 'POST',
    path: '/storeActions/single',
    description: 'Perform Single Store Action (Body Auth)',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iPerformStoreActionSingleWithBodyAuthPayload',
      response: 'iPerformStoreActionSuccessResponse',
      function: 'BitBadgesAPI.performStoreActionSingleWithBodyAuth',
    },
  },
  {
    name: 'perform-store-action-batch-with-body-auth',
    tag: 'stores',
    method: 'POST',
    path: '/storeActions/batch',
    description: 'Perform Batch Store Actions (Body Auth)',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iPerformStoreActionBatchWithBodyAuthPayload',
      response: 'iBatchStoreActionSuccessResponse',
      function: 'BitBadgesAPI.performStoreActionBatchWithBodyAuth',
    },
  },
  {
    name: 'get-dynamic-data-stores',
    tag: 'stores',
    method: 'POST',
    path: '/dynamicStores/fetch',
    description: 'Fetch Dynamic Data Stores - Batch',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetDynamicDataStoresPayload',
      response: 'iGetDynamicDataStoresSuccessResponse',
      function: 'BitBadgesAPI.getDynamicDataStores',
    },
  },
  {
    name: 'search-dynamic-data-stores',
    tag: 'stores',
    method: 'GET',
    path: '/dynamicStores/search',
    description: 'Search Dynamic Data Stores For User',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iSearchDynamicDataStoresPayload',
      response: 'iSearchDynamicDataStoresSuccessResponse',
      function: 'BitBadgesAPI.searchDynamicDataStores',
    },
  },
  {
    name: 'get-dynamic-data-activity',
    tag: 'stores',
    method: 'GET',
    path: '/dynamicStores/activity',
    description: 'Get Dynamic Data Activity',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iGetDynamicDataActivityPayload',
      response: 'iGetDynamicDataActivitySuccessResponse',
      function: 'BitBadgesAPI.getDynamicDataActivity',
    },
  },
  // =========================================================================
  // On-chain dynamic store routes
  // =========================================================================
  {
    name: 'get-on-chain-dynamic-store',
    tag: 'onchain-stores',
    method: 'GET',
    path: '/onChainDynamicStore/{storeId}',
    description: 'Get On-Chain Dynamic Store',
    pathParams: ['storeId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetOnChainDynamicStoreSuccessResponse',
      function: 'BitBadgesAPI.getOnChainDynamicStore',
    },
  },
  {
    name: 'get-on-chain-dynamic-stores-by-creator',
    tag: 'onchain-stores',
    method: 'GET',
    path: '/onChainDynamicStores/by-creator/{address}',
    description: 'Get On-Chain Dynamic Stores by Creator',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetOnChainDynamicStoresByCreatorSuccessResponse',
      function: 'BitBadgesAPI.getOnChainDynamicStoresByCreator',
    },
  },
  {
    name: 'get-on-chain-dynamic-store-value',
    tag: 'onchain-stores',
    method: 'GET',
    path: '/onChainDynamicStore/{storeId}/value/{address}',
    description: 'Get On-Chain Dynamic Store Value',
    pathParams: ['storeId', 'address'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetOnChainDynamicStoreValueSuccessResponse',
      function: 'BitBadgesAPI.getOnChainDynamicStoreValue',
    },
  },
  {
    name: 'get-on-chain-dynamic-store-values-paginated',
    tag: 'onchain-stores',
    method: 'GET',
    path: '/onChainDynamicStore/{storeId}/values',
    description: 'Get On-Chain Dynamic Store Values (Paginated)',
    pathParams: ['storeId'],
    hasBody: false,
    queryParams: [
      { name: 'bookmark', description: 'Pagination bookmark', required: false },
      { name: 'limit', description: 'Number of results to return (1-100)', required: false },
    ],
    sdkLinks: {
      response: 'iGetOnChainDynamicStoreValuesPaginatedSuccessResponse',
      function: 'BitBadgesAPI.getOnChainDynamicStoreValuesPaginated',
    },
  },
  // =========================================================================
  // Utility page routes
  // =========================================================================
  {
    name: 'get-utility-page',
    tag: 'pages',
    method: 'GET',
    path: '/utilityPages/{listingId}',
    description: 'Get Utility Page',
    pathParams: ['listingId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetUtilityPagePayload',
      response: 'iGetUtilityPageSuccessResponse',
      function: 'BitBadgesAPI.getUtilityPage',
    },
  },
  {
    name: 'get-utility-pages',
    tag: 'pages',
    method: 'POST',
    path: '/utilityPages/fetch',
    description: 'Get Utility Pages - Batch',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetUtilityPagesPayload',
      response: 'iGetUtilityPagesSuccessResponse',
      function: 'BitBadgesAPI.getUtilityPages',
    },
  },
  {
    name: 'search-utility-pages',
    tag: 'pages',
    method: 'GET',
    path: '/utilityPages/search',
    description: 'Search Utility Pages',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iSearchUtilityPagesPayload',
      response: 'iSearchUtilityPagesSuccessResponse',
      function: 'BitBadgesAPI.searchUtilityPages',
    },
  },
  {
    name: 'create-utility-page',
    tag: 'pages',
    method: 'POST',
    path: '/utilityPages',
    description: 'Create Utility Page',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCreateUtilityPagePayload',
      response: 'iCreateUtilityPageSuccessResponse',
      function: 'BitBadgesAPI.createUtilityPage',
    },
  },
  {
    name: 'update-utility-page',
    tag: 'pages',
    method: 'PUT',
    path: '/utilityPages',
    description: 'Update Utility Page',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iUpdateUtilityPagePayload',
      response: 'iUpdateUtilityPageSuccessResponse',
      function: 'BitBadgesAPI.updateUtilityPage',
    },
  },
  {
    name: 'delete-utility-page',
    tag: 'pages',
    method: 'DELETE',
    path: '/utilityPages',
    description: 'Delete Utility Page',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iDeleteUtilityPagePayload',
      response: 'iDeleteUtilityPageSuccessResponse',
      function: 'BitBadgesAPI.deleteUtilityPage',
    },
  },
  // =========================================================================
  // DEX, pools, and asset pair routes
  // =========================================================================
  {
    name: 'get-all-pools',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/pools',
    description: 'Get All Pools',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetAllPoolsPayload',
      response: 'iGetAllPoolsSuccessResponse',
      function: 'BitBadgesAPI.getAllPools',
    },
  },
  {
    name: 'get-pool-infos-by-denom',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/pools/byDenom',
    description: 'Get Pool Infos by Denom',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetPoolInfosByDenomPayload',
      response: 'iGetPoolInfosByDenomSuccessResponse',
      function: 'BitBadgesAPI.getPoolInfosByDenom',
    },
  },
  {
    name: 'get-pool-infos-by-assets',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/pools/byAssets',
    description: 'Get Pool Infos by Assets',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetPoolInfosByAssetsPayload',
      response: 'iGetPoolInfosByAssetsSuccessResponse',
      function: 'BitBadgesAPI.getPoolInfosByAssets',
    },
  },
  {
    name: 'get-pool-info-by-id',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/pools/{poolId}',
    description: 'Get Pool Info by ID',
    pathParams: ['version', 'poolId'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetPoolInfoByIdPayload',
      response: 'iGetPoolInfoByIdSuccessResponse',
      function: 'BitBadgesAPI.getPoolInfoById',
    },
  },
  {
    name: 'estimate-swap',
    tag: 'assets',
    method: 'POST',
    path: '/api/{version}/swap/estimate',
    description: 'Estimate Swap',
    pathParams: ['version'],
    hasBody: true,
    sdkLinks: {
      request: 'iEstimateSwapPayload',
      response: 'iEstimateSwapSuccessResponse',
      function: 'BitBadgesAPI.estimateSwap',
    },
  },
  {
    name: 'estimate-swap-legacy',
    tag: 'assets',
    method: 'POST',
    path: '/api/{version}/swaps/estimate',
    description: '[DEPRECATED] Estimate Swap',
    pathParams: ['version'],
    hasBody: true,
    sdkLinks: {
      request: 'iEstimateSwapPayload',
      response: 'iEstimateSwapSuccessResponse',
      function: 'BitBadgesAPI.estimateSwapLegacy',
    },
  },
  {
    name: 'get-asset-pairs',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs',
    description: 'Get Asset Pairs',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetAssetPairsPayload',
      response: 'iGetAssetPairsSuccessResponse',
      function: 'BitBadgesAPI.getAssetPairs',
    },
  },
  {
    name: 'get-top-gainers',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs/topGainers',
    description: 'Get Top Gainers',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetTopGainersPayload',
      response: 'iGetTopGainersSuccessResponse',
      function: 'BitBadgesAPI.getTopGainers',
    },
  },
  {
    name: 'get-top-losers',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs/topLosers',
    description: 'Get Top Losers',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetTopLosersPayload',
      response: 'iGetTopLosersSuccessResponse',
      function: 'BitBadgesAPI.getTopLosers',
    },
  },
  {
    name: 'get-highest-volume',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs/highestVolume',
    description: 'Get Highest Volume',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetHighestVolumePayload',
      response: 'iGetHighestVolumeSuccessResponse',
      function: 'BitBadgesAPI.getHighestVolume',
    },
  },
  {
    name: 'get-by-price',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs/priceSorted',
    description: 'Get by Price',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetByPricePayload',
      response: 'iGetByPriceSuccessResponse',
      function: 'BitBadgesAPI.getByPrice',
    },
  },
  {
    name: 'get-weekly-top-gainers',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs/weeklyTopGainers',
    description: 'Get Weekly Top Gainers',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetWeeklyTopGainersPayload',
      response: 'iGetWeeklyTopGainersSuccessResponse',
      function: 'BitBadgesAPI.getWeeklyTopGainers',
    },
  },
  {
    name: 'get-weekly-top-losers',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs/weeklyTopLosers',
    description: 'Get Weekly Top Losers',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetWeeklyTopLosersPayload',
      response: 'iGetWeeklyTopLosersSuccessResponse',
      function: 'BitBadgesAPI.getWeeklyTopLosers',
    },
  },
  {
    name: 'search-asset-pairs-by-text',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/assetPairs/search',
    description: 'Search Asset Pairs by Text',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iSearchAssetPairsByTextPayload',
      response: 'iSearchAssetPairsByTextSuccessResponse',
      function: 'BitBadgesAPI.searchAssetPairsByText',
    },
  },
  {
    name: 'get-by-denoms',
    tag: 'assets',
    method: 'POST',
    path: '/api/{version}/assetPairs/byDenoms',
    description: 'Get Asset Pairs by Denoms',
    pathParams: ['version'],
    hasBody: true,
    sdkLinks: {
      request: 'iGetByDenomsPayload',
      response: 'iGetByDenomsSuccessResponse',
      function: 'BitBadgesAPI.getByDenoms',
    },
  },
  {
    name: 'get-swap-activities',
    tag: 'assets',
    method: 'GET',
    path: '/swapActivities',
    description: 'Get Swap Activities',
    pathParams: [],
    hasBody: false,
    queryParams: [
      { name: 'bookmark', description: 'Pagination bookmark', required: false },
      { name: 'limit', description: 'Number of results to return (1-100)', required: false },
    ],
    sdkLinks: {
      response: 'iGetSwapActivitiesSuccessResponse',
      function: 'BitBadgesAPI.getSwapActivities',
    },
  },
  {
    name: 'get-swap-assets',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/swap/assets',
    description: 'Get Swap Assets',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSwapAssetsPayload',
      response: 'iGetSwapAssetsSuccessResponse',
      function: 'BitBadgesAPI.getSwapAssets',
    },
  },
  {
    name: 'get-swap-chains',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/swap/chains',
    description: 'Get Swap Chains',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSwapChainsPayload',
      response: 'iGetSwapChainsSuccessResponse',
      function: 'BitBadgesAPI.getSwapChains',
    },
  },
  {
    name: 'get-swap-balances',
    tag: 'assets',
    method: 'POST',
    path: '/api/{version}/swap/balances',
    description: 'Get Swap Balances',
    pathParams: ['version'],
    hasBody: true,
    sdkLinks: {
      request: 'iGetSwapBalancesPayload',
      response: 'iGetSwapBalancesSuccessResponse',
      function: 'BitBadgesAPI.getSwapBalances',
    },
  },
  {
    name: 'track-swap',
    tag: 'assets',
    method: 'POST',
    path: '/api/{version}/swap/track',
    description: 'Track Swap Transaction',
    pathParams: ['version'],
    hasBody: true,
    sdkLinks: {
      request: 'iTrackSwapPayload',
      response: 'iTrackSwapSuccessResponse',
      function: 'BitBadgesAPI.trackSwap',
    },
  },
  {
    name: 'get-swap-status',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/swap/status',
    description: 'Get Swap Transaction Status',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSwapStatusPayload',
      response: 'iGetSwapStatusSuccessResponse',
      function: 'BitBadgesAPI.getSwapStatus',
    },
  },
  {
    name: 'get-skip-assets',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/skip/assets',
    description: '[DEPRECATED] Get Skip:Go Assets',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSkipAssetsPayload',
      response: 'iGetSkipAssetsSuccessResponse',
      function: 'BitBadgesAPI.getSkipAssets',
    },
  },
  {
    name: 'get-skip-chains',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/skip/chains',
    description: '[DEPRECATED] Get Skip:Go Chains',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSkipChainsPayload',
      response: 'iGetSkipChainsSuccessResponse',
      function: 'BitBadgesAPI.getSkipChains',
    },
  },
  {
    name: 'get-skip-balances',
    tag: 'assets',
    method: 'POST',
    path: '/api/{version}/skip/balances',
    description: '[DEPRECATED] Get Skip:Go Balances',
    pathParams: ['version'],
    hasBody: true,
    sdkLinks: {
      request: 'iGetSkipBalancesPayload',
      response: 'iGetSkipBalancesSuccessResponse',
      function: 'BitBadgesAPI.getSkipBalances',
    },
  },
  {
    name: 'track-skip-tx',
    tag: 'assets',
    method: 'POST',
    path: '/api/{version}/skip/v2/tx/track',
    description: '[DEPRECATED] Track Skip:Go Transaction',
    pathParams: ['version'],
    hasBody: true,
    sdkLinks: {
      request: 'iTrackSkipTxPayload',
      response: 'iTrackSkipTxSuccessResponse',
      function: 'BitBadgesAPI.trackSkipTx',
    },
  },
  {
    name: 'get-skip-tx-status',
    tag: 'assets',
    method: 'GET',
    path: '/api/{version}/skip/v2/tx/status',
    description: '[DEPRECATED] Get Skip:Go Transaction Status',
    pathParams: ['version'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetSkipTxStatusPayload',
      response: 'iGetSkipTxStatusSuccessResponse',
      function: 'BitBadgesAPI.getSkipTxStatus',
    },
  },
  {
    name: 'get-intents',
    tag: 'assets',
    method: 'GET',
    path: '/intents/{address}',
    description: 'Get Exchange-Approval Intents',
    pathParams: ['address'],
    hasBody: false,
    sdkLinks: {
      request: 'iGetIntentsPayload',
      response: 'iGetIntentsSuccessResponse',
      function: 'BitBadgesAPI.getIntents',
    },
  },
  {
    name: 'get-liquidity-pair-price-history',
    tag: 'assets',
    method: 'GET',
    path: '/liquidityPairPriceHistory',
    description: 'Get Liquidity Pair Price History',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iGetLiquidityPairPriceHistoryPayload',
      response: 'iGetLiquidityPairPriceHistorySuccessResponse',
      function: 'BitBadgesAPI.getLiquidityPairPriceHistory',
    },
  },
  {
    name: 'get-pools-batch',
    tag: 'assets',
    method: 'POST',
    path: '/pools/batch',
    description: 'Get Pools (Batch)',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetPoolsBatchPayload',
      response: 'iGetPoolsBatchSuccessResponse',
      function: 'BitBadgesAPI.getPoolsBatch',
    },
  },
  {
    name: 'browse-intents',
    tag: 'assets',
    method: 'GET',
    path: '/intents',
    description: 'Browse All Intents',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iGetIntentsPayload',
      response: 'iGetIntentsSuccessResponse',
      function: 'BitBadgesAPI.browseIntents',
    },
  },
  // =========================================================================
  // Miscellaneous routes
  // =========================================================================
  {
    name: 'get-status',
    tag: 'misc',
    method: 'GET',
    path: '/status',
    description: 'Get Status',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iGetStatusPayload',
      response: 'iGetStatusSuccessResponse',
      function: 'BitBadgesAPI.getStatus',
    },
  },
  {
    name: 'get-prompt-skill',
    tag: 'misc',
    method: 'GET',
    path: '/promptSkill/{promptSkillId}',
    description: 'Get Prompt Skill',
    pathParams: ['promptSkillId'],
    hasBody: false,
    sdkLinks: {
      response: 'iGetPromptSkillSuccessResponse',
      function: 'BitBadgesAPI.getPromptSkill',
    },
  },
  {
    name: 'search-prompt-skills',
    tag: 'misc',
    method: 'GET',
    path: '/promptSkills/search',
    description: 'Search Prompt Skills',
    pathParams: [],
    hasBody: false,
    sdkLinks: {
      request: 'iSearchPromptSkillsPayload',
      response: 'iSearchPromptSkillsSuccessResponse',
      function: 'BitBadgesAPI.searchPromptSkills',
    },
  },
  {
    name: 'fetch-prompt-skills',
    tag: 'misc',
    method: 'POST',
    path: '/promptSkills/fetch',
    description: 'Fetch Prompt Skills (Batch)',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iFetchPromptSkillsPayload',
      response: 'iFetchPromptSkillsSuccessResponse',
      function: 'BitBadgesAPI.fetchPromptSkills',
    },
  },
  {
    name: 'create-prompt-skill',
    tag: 'misc',
    method: 'POST',
    path: '/promptSkills',
    description: 'Create Prompt Skill',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCreatePromptSkillPayload',
      response: 'iCreatePromptSkillSuccessResponse',
      function: 'BitBadgesAPI.createPromptSkill',
    },
  },
  {
    name: 'update-prompt-skill',
    tag: 'misc',
    method: 'PUT',
    path: '/promptSkills',
    description: 'Update Prompt Skill',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iUpdatePromptSkillPayload',
      response: 'iUpdatePromptSkillSuccessResponse',
      function: 'BitBadgesAPI.updatePromptSkill',
    },
  },
  {
    name: 'delete-prompt-skill',
    tag: 'misc',
    method: 'DELETE',
    path: '/promptSkills',
    description: 'Delete Prompt Skill',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iDeletePromptSkillPayload',
      response: 'iDeletePromptSkillSuccessResponse',
      function: 'BitBadgesAPI.deletePromptSkill',
    },
  },
  {
    name: 'create-api-key',
    tag: 'misc',
    method: 'POST',
    path: '/apiKeys',
    description: 'Create API Key',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iCreateApiKeyPayload',
      response: 'iCreateApiKeySuccessResponse',
      function: 'BitBadgesAPI.createApiKey',
    },
  },
  {
    name: 'delete-api-key',
    tag: 'misc',
    method: 'DELETE',
    path: '/apiKeys',
    description: 'Delete API Key',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iDeleteApiKeyPayload',
      response: 'iDeleteApiKeySuccessResponse',
      function: 'BitBadgesAPI.deleteApiKey',
    },
  },
  {
    name: 'fetch-api-keys',
    tag: 'misc',
    method: 'POST',
    path: '/apiKeys/fetch',
    description: 'Fetch API Keys',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetApiKeysPayload',
      response: 'iGetApiKeysSuccessResponse',
      function: 'BitBadgesAPI.fetchApiKeys',
    },
  },
  {
    name: 'rotate-api-key',
    tag: 'misc',
    method: 'POST',
    path: '/apiKeys/rotate',
    description: 'Rotate API Key',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iRotateApiKeyPayload',
      response: 'iRotateApiKeySuccessResponse',
      function: 'BitBadgesAPI.rotateApiKey',
    },
  },
  {
    name: 'browse',
    tag: 'misc',
    method: 'POST',
    path: '/browse',
    description: 'Browse Collections, Tokens, Users',
    pathParams: [],
    hasBody: true,
    sdkLinks: {
      request: 'iGetBrowsePayload',
      response: 'iGetBrowseSuccessResponse',
      function: 'BitBadgesAPI.browse',
    },
  },
];
