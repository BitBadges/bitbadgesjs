/**
 * Pure generation logic for the ROUTES data array at src/cli/commands/api-routes.ts.
 *
 * Consumed by:
 *   - scripts/generate-api-routes.ts (the CLI entry point that writes the file)
 *   - src/cli/commands/api-routes-drift.spec.ts (the CI guard that fails when
 *     the checked-in file drifts from openapitypes-helpers/routes.yaml)
 *
 * Takes the raw routes.yaml content as input and returns the full file content
 * as a string — no filesystem access, so it stays trivially testable.
 */
...
export function parseRoutes(yamlRaw: string): ParsedRoute[] {
...
/** Full content of src/cli/commands/api-routes.ts for the given routes.yaml content. */
export function generateApiRoutesFileContent(yamlRaw: string): string {
  return generateFile(parseRoutes(yamlRaw));
}
