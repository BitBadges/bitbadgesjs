...
import { loadSessionFromDisk, saveSessionToDisk } from '../../builder/session/fileStore.js';
...
export const toolCommand = new Command('tool')
  ...
  .action(
    async (
      toolName: string,
      opts: { args?: string; argsFile?: string; session?: string; raw?: boolean; local?: boolean; testnet?: boolean; url?: string }
    ) => {
      ...
      const structuredFail = result.result && typeof result.result === 'object' && (result.result as any).success === false;
