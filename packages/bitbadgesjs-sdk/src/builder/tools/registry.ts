...
export const toolRegistry: Record<string, ToolEntry> = {
  ...
  validate_intent: entry({ name: 'validate_intent', description: 'Validate an intent sidecar, reject contradictory constraints, and list decisions requiring clarification.', inputSchema: { type: 'object', properties: { intent: { type: 'object' } }, required: ['intent'] } },
    (args) => { const { intent } = z.object({ intent: z.unknown() }).strict().parse(args); const parsed = parseIntent(intent); return { intent: parsed, intentId: artifactIdentity(parsed), clarificationRequired: parsed.requirements.length === 0 || parsed.unresolvedDecisions.length > 0 || parsed.requirements.some(r => r.value === null || r.kind === 'unsupported') }; }),
