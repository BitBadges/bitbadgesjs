...
const intent = {
  version: 1,
  requirements: [{ id: 'cap', source: 'user', kind: 'supply', value: { maxSupplyPerId: '10' } }],
  unresolvedDecisions: []
};
const scenario = { version: 1, id: 'bound', steps: [{ id: 'create', message }] };
describe('artifact-bound lifecycle intent checks', () => {
  it('rejects unsupported batch representation before launching the single-message runner', async () => {
    const run = jest
      .spyOn(runner, 'runLifecycle')
      .mockResolvedValue({ status: 'satisfied', passed: true, coverage: { executed: ['module'], excluded: [] } } as any);
    try {
      await expect(verifyLifecycleIntent({ artifact, intent, scenario: { ...scenario, steps: [{ messages: [message] }] } })).rejects.toThrow();
  ...
  it('rejects a substituted artifact before executing any scenario', async () => {
    ...
    try {
      await expect(
        verifyLifecycleIntent({ artifact, intent, scenario: { ...scenario, steps: [{ id: 'create', message: { ...message, value: {} } }] } })
  ...
  it('retains static evidence and marks requirement-to-scenario coverage unverified', async () => {
    const run = jest
      ...
      .mockResolvedValue({
        ...
        coverage: { executed: ['module'], excluded: [] }
      } as any);
    try {
  ...
  it('never hides a scenario failure behind a satisfied static artifact', async () => {
    const run = jest
      .spyOn(runner, 'runLifecycle')
      .mockResolvedValue({ status: 'violated', passed: false, coverage: { executed: ['module'], excluded: [] } } as any);
    try {
      expect((await verifyLifecycleIntent({ artifact, intent, scenario })).status).toBe('violated');
    } finally {
      run.mockRestore();
    }
  });
