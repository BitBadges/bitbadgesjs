...
// System approvals generated by SDK builders / AI builder tools that
// don't need a custom display name — the ID itself is self-documenting
// and the frontend renders a canonical label for each. Matched by
// PREFIX because the AI builder tool appends a random hex suffix
// (e.g. `smart-token-backing_5745316c`).
const KNOWN_SYSTEM_APPROVAL_PREFIXES = [
  ...
  'vault-emergency-migration',
  'manager-add',
  'manager-remove'
];

function isKnownSystemApproval(id: string): boolean {
  return KNOWN_SYSTEM_APPROVAL_PREFIXES.some(
    (prefix) => id === prefix || id.startsWith(prefix + '_') || id.startsWith(prefix + '-')
  );
}
...
export const metadataChecks: UxCheck[] = [
  ...
  (value) => {
    const out: Finding[] = [];
    // metadataPlaceholders lives at value._meta.metadataPlaceholders —
    // per-msg sidecar keyed by URI strings inside the msg body.
    // review-normalize.ts reattaches `_meta` after the proto class
    // conversion strips it, so this lookup is safe after normalization.
  ...
  (value) => {
    ...
    const metadataPlaceholders: Record<string, { name?: string }> = value?._meta?.metadataPlaceholders || {};
    const unnamed = approvals.filter((a: any) => {
      const id = a.approvalId || '';
      if (id.startsWith('default-') || id === '') return false;
      if (isKnownSystemApproval(id)) return false;
  ...
  (value) => {
    ...
    const metadataPlaceholders: Record<string, { image?: string }> = value?._meta?.metadataPlaceholders || {};
