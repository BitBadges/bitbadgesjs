...
Browser transaction records live under the configured CLI directory with 0700 directories and 0600 files. `bb dev requests status` is local inspection, not chain verification. `resume` returns the original URL only after a nonce-bound check of the live listener; there is no background daemon or automatic replacement transaction. Expiry and listener loss remain unknown outcomes until independently reconciled.
