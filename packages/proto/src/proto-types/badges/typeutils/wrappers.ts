import * as balances from '../../../proto/badges/balances_pb';
import * as metadata from '../../../proto/badges/metadata_pb';
import * as permissions from '../../../proto/badges/permissions_pb';
import * as timelines from '../../../proto/badges/timelines_pb';
import * as transfers from '../../../proto/badges/transfers_pb';
...
export function getWrappedBadgeIds<T extends NumberType>(badgeIds: UintRange<T>[]) {
  const wrappedBadgeIds: balances.UintRange[] = []
  for (const range of badgeIds) {
    wrappedBadgeIds.push(
      new balances.UintRange({
...
export function getWrappedBadgeMetadata<T extends NumberType>(badgeUris: BadgeMetadata<T>[]) {
  const wrappedBadgeMetadatas: metadata.BadgeMetadata[] = [];
  for (const badgeUri of badgeUris) {
    wrappedBadgeMetadatas.push(
      new metadata.BadgeMetadata({
        uri: badgeUri.uri,
...
export function getWrappedBalances<T extends NumberType>(balanceArr: Balance<T>[]) {
  const formattedBalances: balances.Balance[] =
    []
...
export function getWrappedProof(proof: MerkleProof) {
  const wrappedAunts: transfers.MerklePathItem[] = []
  for (const aunt of proof.aunts) {
    wrappedAunts.push(
      new transfers.MerklePathItem({
        aunt: aunt.aunt,
  ...
  const wrappedProof: transfers.MerkleProof =
    new transfers.MerkleProof({
      aunts: wrappedAunts,
      leaf: proof.leaf,
    })
  return wrappedProof
...
export function getWrappedTransfers<T extends NumberType>(transfersArr: Transfer<T>[]) {
  const wrappedTransfers: transfers.Transfer[] = []
  for (const transfer of transfersArr) {
    ...
    wrappedTransfers.push(
      new transfers.Transfer({
        ...
        precalculateBalancesFromApproval: transfer.precalculateBalancesFromApproval ? new transfers.ApprovalIdentifierDetails({ ...transfer.precalculateBalancesFromApproval }) : undefined,
        prioritizedApprovals: transfer.prioritizedApprovals ? transfer.prioritizedApprovals.map((prioritizedApproval) => {
          return new transfers.ApprovalIdentifierDetails({
            ...prioritizedApproval,
          })
        }) : undefined,
      }),
...
export function getWrappedTimedUpdatePermission<T extends NumberType>(permissionsArr: TimedUpdatePermission<T>[]) {
  return permissionsArr.map((canArchiveCollection) => {
    return new permissions.TimedUpdatePermission({
...
export function getWrappedTimedUpdateWithBadgeIdsPermission<T extends NumberType>(permissionsArr: TimedUpdateWithBadgeIdsPermission<T>[]) {
  return permissionsArr.map((canArchiveCollection) => {
    return new permissions.TimedUpdateWithBadgeIdsPermission({
      ...canArchiveCollection,
      timelineTimes: getWrappedBadgeIds(canArchiveCollection.timelineTimes),
...
export function getWrappedBalancesActionPermission<T extends NumberType>(permissionsArr: BalancesActionPermission<T>[]) {
  return permissionsArr.map((canArchiveCollection) => {
    return new permissions.BalancesActionPermission({
      ...canArchiveCollection,
...
export function getWrappedCollectionApprovalPermission<T extends NumberType>(permissionsArr: CollectionApprovalPermission<T>[]) {
  return permissionsArr.map((canArchiveCollection) => {
    return new permissions.CollectionApprovalPermission({
      ...canArchiveCollection,
...
export function getWrappedUserPermissions<T extends NumberType>(userPermissions: UserPermissions<T>) {
  return new permissions.UserPermissions({
    ...
    canUpdateIncomingApprovals: userPermissions.canUpdateIncomingApprovals.map((canUpdateIncomingApproval) => {
      return new permissions.UserIncomingApprovalPermission({
    ...
    canUpdateOutgoingApprovals: userPermissions.canUpdateOutgoingApprovals.map((canUpdateOutgoingApproval) => {
      return new permissions.UserOutgoingApprovalPermission({
...
export function getWrappedOutgoingTransfers<T extends NumberType>(outgoingApprovals: UserOutgoingApproval<T>[]) {
  return outgoingApprovals.map((outgoingTransfer) => {
    ...
    return new transfers.UserOutgoingApproval({
      ...outgoingTransfer,
      ...
      approvalCriteria: approvalCriteria ? new transfers.OutgoingApprovalCriteria({
        ...approvalCriteria,
        mustOwnBadges: approvalCriteria.mustOwnBadges ? approvalCriteria.mustOwnBadges.map((mustOwnBadge) => {
          return new balances.MustOwnBadges({
            ...mustOwnBadge,
        ...
        merkleChallenge: approvalCriteria.merkleChallenge ? new transfers.MerkleChallenge({
        ...
        predeterminedBalances: approvalCriteria.predeterminedBalances ? new transfers.PredeterminedBalances({
          ...approvalCriteria.predeterminedBalances,
          incrementedBalances: new transfers.IncrementedBalances({
          ...
          orderCalculationMethod: new transfers.PredeterminedOrderCalculationMethod({
          ...
          manualBalances: approvalCriteria.predeterminedBalances.manualBalances.map((manualBalance) => {
            return new transfers.ManualBalances({
              ...manualBalance,
              balances: getWrappedBalances(manualBalance.balances),
            })
          }),
        }) : undefined,
        approvalAmounts: approvalCriteria.approvalAmounts ? new transfers.ApprovalAmounts({
        ...
        maxNumTransfers: approvalCriteria.maxNumTransfers ? new transfers.MaxNumTransfers({
...
export function getWrappedIncomingTransfers<T extends NumberType>(incomingApprovals: UserIncomingApproval<T>[]) {
  return incomingApprovals.map((outgoingTransfer) => {
    ...
    const approvalCriteria = outgoingTransfer.approvalCriteria ?? DefaultNullApprovalCriteria;
    return new transfers.UserIncomingApproval({
      ...outgoingTransfer,
      transferTimes: getWrappedBadgeIds(outgoingTransfer.transferTimes),
      badgeIds: getWrappedBadgeIds(outgoingTransfer.badgeIds),
      ownershipTimes: getWrappedBadgeIds(outgoingTransfer.ownershipTimes),

      approvalCriteria: approvalCriteria ? new transfers.IncomingApprovalCriteria({
        ...approvalCriteria,
        mustOwnBadges: approvalCriteria.mustOwnBadges ? approvalCriteria.mustOwnBadges.map((mustOwnBadge) => {
          return new balances.MustOwnBadges({
            ...mustOwnBadge,
...
export function getWrappedCollectionApprovals<T extends NumberType>(collectionApprovals: CollectionApproval<T>[]) {
  return collectionApprovals.map((outgoingTransfer) => {
    const approvalCriteria = outgoingTransfer.approvalCriteria ?? DefaultNullApprovalCriteria;
    return new transfers.CollectionApproval({
      ...outgoingTransfer,
      transferTimes: getWrappedBadgeIds(outgoingTransfer.transferTimes),
      badgeIds: getWrappedBadgeIds(outgoingTransfer.badgeIds),
      ownershipTimes: getWrappedBadgeIds(outgoingTransfer.ownershipTimes),
      approvalCriteria: approvalCriteria ? new transfers.ApprovalCriteria({
        ...approvalCriteria,
        mustOwnBadges: approvalCriteria.mustOwnBadges ? approvalCriteria.mustOwnBadges.map((mustOwnBadge) => {
          return new balances.MustOwnBadges({
            ...mustOwnBadge,
...
export function getWrappedManagerTimeline(managerTimeline: ManagerTimeline<NumberType>[]) {
  return managerTimeline.map((managerTimeline) => new timelines.ManagerTimeline({
    ...managerTimeline,
...
export function getWrappedCollectionMetadataTimeline(collectionMetadataTimeline: CollectionMetadataTimeline<NumberType>[]) {
  return collectionMetadataTimeline.map((collectionMetadataTimeline) => new timelines.CollectionMetadataTimeline({
    ...collectionMetadataTimeline,
    timelineTimes: getWrappedBadgeIds(collectionMetadataTimeline.timelineTimes),
    collectionMetadata: new metadata.CollectionMetadata({ ...collectionMetadataTimeline.collectionMetadata }),
  }))
}

export const getWrappedBadgeMetadataTimeline = (badgeMetadataTimeline: BadgeMetadataTimeline<NumberType>[]) => badgeMetadataTimeline.map((badgeMetadataTimeline) => new timelines.BadgeMetadataTimeline({
  ...badgeMetadataTimeline,
  timelineTimes: getWrappedBadgeIds(badgeMetadataTimeline.timelineTimes),
  badgeMetadata: badgeMetadataTimeline.badgeMetadata.map((badgeMetadata) => new metadata.BadgeMetadata({
    ...badgeMetadata,
...
export function getWrappedOffChainBalancesMetadataTimeline(offChainBalancesMetadataTimeline: OffChainBalancesMetadataTimeline<NumberType>[]) {
  return offChainBalancesMetadataTimeline.map((offChainBalancesMetadataTimeline) => new timelines.OffChainBalancesMetadataTimeline({
    ...offChainBalancesMetadataTimeline,
    timelineTimes: getWrappedBadgeIds(offChainBalancesMetadataTimeline.timelineTimes),
    offChainBalancesMetadata: new metadata.OffChainBalancesMetadata({ ...offChainBalancesMetadataTimeline.offChainBalancesMetadata }),
  }))
}

export function getWrappedCustomDataTimeline(customDataTimeline: CustomDataTimeline<NumberType>[]) {
  return customDataTimeline.map((customDataTimeline) => new timelines.CustomDataTimeline({
    ...customDataTimeline,
...
export function getWrappedInheritedBalancesTimeline(inheritedBalancesTimeline: InheritedBalancesTimeline<NumberType>[]) {
  return inheritedBalancesTimeline.map((inheritedBalancesTimeline) => new timelines.InheritedBalancesTimeline({
    ...inheritedBalancesTimeline,
    timelineTimes: getWrappedBadgeIds(inheritedBalancesTimeline.timelineTimes),
    inheritedBalances: inheritedBalancesTimeline.inheritedBalances.map((inheritedBalance) => new balances.InheritedBalance({
      ...inheritedBalance,
...
export function getWrappedStandardsTimeline(standardsTimeline: StandardsTimeline<NumberType>[]) {
  return standardsTimeline.map((standardsTimeline) => new timelines.StandardsTimeline({
    ...standardsTimeline,
...
export function getWrappedIsArchivedTimeline(isArchivedTimeline: IsArchivedTimeline<NumberType>[]) {
  return isArchivedTimeline.map((isArchivedTimeline) => new timelines.IsArchivedTimeline({
    ...isArchivedTimeline,
